...
const MegamenuModal = ({ setRefresh }) => {
  ...
  const handleSave = async () => {
    if (validateFields()) {
      try {
        const response = await axios.post( `${process.env.REACT_APP_BASE}/api/header_mega_menu_handler/`, formData);
        if (response.status === 200 || response.status === 201) {
          toast.success("Data saved successfully!");
          setRefresh && setRefresh((prev) => !prev); // Trigger parent component refresh
          handleClose();
        } else {
