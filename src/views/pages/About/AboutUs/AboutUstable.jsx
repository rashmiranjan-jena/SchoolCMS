...
const AboutUstable = ({ refresh }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_BASE}/api/about-us`);
      ...
      if (result.isConfirmed) {
        try {
          const response = await axios.put(
            `${process.env.REACT_APP_BASE}/api/about-us/?about_us_id=${id}`,
            { status }
          );
          if (response.status === 200) {
            setData((prevData) =>
              prevData.map((item) =>
                item.about_us_id === id ? { ...item, status } : item
              )
            );
  ...
  const handleDelete = async (abouUsId) => {
    Swal.fire({
      title: "Are you sure?",
          ...
          if (result.isConfirmed) {
            try {
              const response = await axios.delete(
                `${process.env.REACT_APP_BASE}/api/about-us/?about_us_id=${abouUsId}`
              );
              if (response.status === 204) {
                const updatedData = data.filter((item) => item.about_us_id !== abouUsId);
                setData(updatedData);
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text, record) => {
        const menu = (
          <Menu onClick={({ key }) => handleStatusChange(record.about_us_id, key)}>
      ...
      render: (_, record) => (
        <div className="action-icons">
          ...
          <Button
            ...
            title="Delete"
            onClick={() => handleDelete(record.about_us_id)}
          />
        </div>
      ),
    },
