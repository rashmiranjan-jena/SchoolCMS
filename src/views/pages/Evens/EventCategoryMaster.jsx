...
// import AddEventCategoryMaster from "../../../components/modelpopup/AddEventCategoryMaster";
import AddEventCategoryMaster from "../../../components/modelpopup/AddEventCategoryMaster";
...
const getEventCatData = async() =>{
 const result = await get('events-category-master/');
 setData(result)
}
  ...
  const handleStatusChange = async (id, newStatus) => {
    ...
    try {
      await update(`events-category-master/?event_category_master_id=`,id,status);
      getEventCatData()
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmed) {
      ...
      if (finalConfirmed) {
        try {
          const response = await _delete(`events-category-master/?event_category_master_id=`,id);
  ...
  const columns = [
    {
      title: "Sl No.",
      dataIndex: "event_category_master_id",
      render: (text) => <span>{text}</span>,
      sorter: (a, b) => a.event_category_master_id - b.event_category_master_id,
    },
    {
    ...
    },
    {
      ...
      dataIndex: "photo",
      key: "photo",
      render: (photo) => <img src={`${imgURL}${photo}`} alt="Event Cat photo" style={{ width: "50px", height: "50px" }} />,
    },
        ...
        <div className="dropdown action-label">
          ...
          <div className="dropdown-menu">
            <Link
              ...
              to="#"
              onClick={() => handleStatusChange(record.event_category_master_id, "Published")}
            >
            ...
            <Link
              className="dropdown-item"
              to="#"
              onClick={() => handleStatusChange(record.event_category_master_id, "Unpublished")}
            >
      ...
      render: (_, record) => (
        <div className="action-icons">
          ...
          <Link
            ...
            className="action-icon"
            title="delete"
            onClick={()=>handleDelete(record.event_category_master_id)}
          >
  ...
  return (
    <div className="page-wrapper">
      <div className="content container-fluid">
        <Breadcrumbs
          maintitle="EventCategoryMaster"
          title="Events"
          subtitle="Event Category"
          modal="#add_EventCategoryMaster"
          name="EventCategoryMaster"
          Linkname="/AddEventCategoryMaster"
        />
          ...
          <div className="col-md-12">
            <div className="table-responsive">
              <Table
                ...
                rowKey={(record) => record.event_category_master_id}
              />
            </div>
          </div>
        </div>
      </div>
      <AddEventCategoryMaster />
