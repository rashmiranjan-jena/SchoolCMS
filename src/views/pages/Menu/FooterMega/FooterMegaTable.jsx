...
const FooterMegaTable = ({ refresh }) => {
  const [data, setData] = useState([]);

  useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_BASE}/api/footer_mega_menu_handler/`)
      .then((response) => {
        const formattedData = response.data.map((item, index) => ({
          srNo: index + 1, 
          menuname: item.name,
          megamenuname: item.menu_name, 
          order: item.order,
          slug: item.slug,
          status: "Unpublished", 
          arno: item.id, 
...
export default FooterMegaTable;
