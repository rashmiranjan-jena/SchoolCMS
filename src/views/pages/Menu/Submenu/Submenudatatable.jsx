...
const Submenudatatable = ({ refresh }) => {
  const [isModalVisible, setIsModalVisible] = useState(false);
  const [data, setData] = useState([]);

  useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_BASE}/api/menu/`)
      .then((response) => {
        const formattedData = response.data.flatMap((club) =>
          club.members.map((member) => ({
            arno: member.member_id,
            menuname: club.club_name,
            order: member.designation,
            slug: member.member_name,
...
export default Submenudatatable;
