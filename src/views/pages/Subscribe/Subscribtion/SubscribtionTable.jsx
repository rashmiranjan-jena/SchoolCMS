...
const SubscribtionTable = () => {
  const [data, setData] = useState([]);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_BASE}/api/subscribtion/`);
      ...
      if (result.isConfirmed) {
        try {
          const response = await axios.put(
            `${process.env.REACT_APP_BASE}/api/subscribtion/?subscribtion_id=${id}`,
            { status }
          );
          if (response.status === 200) {
...
export default SubscribtionTable;
