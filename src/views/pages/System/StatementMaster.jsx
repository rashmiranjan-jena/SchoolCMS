...
import AddStatementMaster from "../../../components/modelpopup/AddStatementMaster";
...
const getStatementData = async() =>{
 const result = await get('statement-master/');
 setData(result)
}
  ...
  const handleStatusChange = async (id, newStatus) => {
    ...
    try {
      await update(`statement-master/?statement_master_id=`,id,status);
      getStatementData()
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmed) {
      ...
      if (finalConfirmed) {
        try {
          const response = await _delete(`statement-master/?statement_master_id=`,id);
  ...
  const columns = [
    {
      title: "SL No.",
      dataIndex: "statement_master_id",
      render: (text, record, index) => index + 1, 
      sorter: (a, b) => a.statement_master_id - b.statement_master_id,
    },
    {
      ...
      <div className="dropdown action-label">
        ...
        <div className="dropdown-menu">
          <Link
            ...
            to="#"
            onClick={() => handleStatusChange(record.statement_master_id, "Published")}
          >
          ...
          <Link
            className="dropdown-item"
            to="#"
            onClick={() => handleStatusChange(record.statement_master_id, "Unpublished")}
          >
      ...
      render: (_, record) => (
        <div className="action-icons">
          ...
          <Link
            ...
            className="action-icon"
            title="delete"
            onClick={()=>handleDelete(record.statement_master_id)}
          >
    ...
    <div className="page-wrapper">
      <div className="content container-fluid">
        <Breadcrumbs
          maintitle="Statement"
          title="Statement"
          subtitle="Statement Master"
          modal="#add_Statement"
          name="Statement Master"
          Linkname="/AddStatement"
        />
          ...
          <div className="col-md-12">
            <div className="table-responsive">
              <Table
                ...
                rowKey={(record) => record.statement_master_id}
              />
            </div>
          </div>
        </div>
...
export default StatementMaster;
