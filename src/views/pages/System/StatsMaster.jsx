...
import AddStatsMaster from "../../../components/modelpopup/AddStatsMaster";
...
const StatsMaster = () => {
  ...
  const getStatementData = async() =>{
    const result = await get('stats-master/');
    setData(result)
   }
  ...
  const handleStatusChange = async (id, newStatus) => {
    ...
    try {
      await update(`stats-master/?stats_master_id=`,id,status);
      getStatementData()
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmed) {
      ...
      if (finalConfirmed) {
        try {
          const response = await _delete(`stats-master/?stats_master_id=`,id);
  ...
  const columns = [
    {
      title: "SL No.",
      dataIndex: "stats_master_id",
      render: (text, record, index) => index + 1, 
      sorter: (a, b) => a.stats_master_id - b.stats_master_id,
    },
    {
      title: "label  ",
      dataIndex: "lebel",
      render: (text) => <span>{text}</span>,
      sorter: (a, b) => a.lebel.localeCompare(b.lebel),
    },
    {
      title: "Stats",
      dataIndex: "Stats",
        ...
        <div className="dropdown action-label">
          ...
          <div className="dropdown-menu">
            <Link
              ...
              to="#"
              onClick={() => handleStatusChange(record.stats_master_id, "Published")}
            >
            ...
            <Link
              className="dropdown-item"
              to="#"
              onClick={() => handleStatusChange(record.stats_master_id, "Unpublished")}
            >
        ...
        render: (_, record) => (
          <div className="action-icons">
            ...
            <Link
              ...
              className="action-icon"
              title="delete"
              onClick={()=>handleDelete(record.stats_master_id)}
            >
          ...
          <div className="col-md-12">
            <div className="table-responsive">
              <Table
                ...
                rowKey={(record) => record.stats_master_id}
              />
            </div>
          </div>
        </div>
