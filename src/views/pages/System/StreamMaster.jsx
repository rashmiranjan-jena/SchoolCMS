...
import AddStreamMaster from "../../../components/modelpopup/AddStreamMaster";
...
const [streamData,setStreamData] = useState([])
  ...
  const getStreamData= async() =>{
    const response = await get(`stream-master/`);
  ...
  const handleStatusChange = async (id, newStatus) => {
    ...
    try {
      await update(`stream-master/?stream_master_id=`,id,status);
      getStreamData()
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmed) {
      ...
      if (finalConfirmed) {
        try {
          const response = await _delete(`stream-master/?stream_master_id=`,id);
  ...
  const columns = [
    {
      title: "Sl No.",
      dataIndex: "stream_master_id",
      render: (text, record, index) => index + 1, 
      sorter: (a, b) => a.stream_master_id - b.stream_master_id,
    },
    {
      title: "Stream Name  ",
      dataIndex: "stream_name",
      render: (text) => <span>{text}</span>,
      sorter: (a, b) => a.StreamName.localeCompare(b.StreamName),
    },
        ...
        <div className="dropdown action-label">
          ...
          <div className="dropdown-menu">
            <Link
              ...
              to="#"
              onClick={() => handleStatusChange(record.stream_master_id, "Published")}
            >
            ...
            <Link
              className="dropdown-item"
              to="#"
              onClick={() => handleStatusChange(record.stream_master_id, "Unpublished")}
            >
      ...
      render: (_, record) => (
        <div className="action-icons">
          ...
          <Link
            to="#"
            className="action-icon"
            onClick={() => handleDelete(record.stream_master_id)}
            title="Delete"
          >
  ...
  return (
    <div className="page-wrapper">
      <div className="content container-fluid">
        <Breadcrumbs
          maintitle="StreamMaster"
          title="StreamMaster"
          subtitle="StreamMaster"
          modal="#add_StreamMaster"
          name="StreamMaster"
          Linkname="/AddStreamMaster"
        />
          ...
          <div className="col-md-12">
            <div className="table-responsive">
              <Table
                ...
                rowKey={(record) => record.stream_master_id}
              />
            </div>
          </div>
        </div>
